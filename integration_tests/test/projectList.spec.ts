...
import { API_URL, getProjectsGetUrl, getProjectsDeleteUrl } from '~integration_tests/utils/data';
...
test('can delete project', async ({ projectListPage, page }) => {
  await page.route(getProjectsDeleteUrl(API_URL, project.id), route => route.fulfill({
    status: 200,
    body: JSON.stringify(project),
  }));
  ...
  await expect(projectListPage.notification.message).toHaveText('Project name deleted')
});
