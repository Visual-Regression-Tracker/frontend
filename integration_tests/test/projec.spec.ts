...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(getBuildsGetUrl(project.id), (route) =>
    route.fulfill({
      body: JSON.stringify({
        data: [TEST_BUILD_FAILED, TEST_BUILD_PASSED, TEST_BUILD_UNRESOLVED],
        skip: 0,
        take: 10,
        total: 3,
      }),
  ...
  await page.route(`**/builds/${TEST_BUILD_FAILED.id}`, (route) =>
    route.fulfill({
      body: JSON.stringify(TEST_BUILD_FAILED),
    })
  );
  await page.route(`**/test-runs?buildId=${TEST_BUILD_FAILED.id}`, (route) =>
    route.fulfill({
      body: JSON.stringify([
        TEST_UNRESOLVED,
        TEST_RUN_APPROVED,
  ...
  await page.route(`**/test-runs/${TEST_UNRESOLVED.id}`, (route) =>
    route.fulfill({
  ...
  await page.route(`**/diff.png`, (route) =>
    route.fulfill({
      path: "integration_tests/images/diff.png",
    })
  );
  await page.route(`**/image.png`, (route) =>
    route.fulfill({
